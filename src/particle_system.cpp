...
void ParticleSystem::draw() {
    if (particles.empty()) return;  // 👈 DODAJ TO
...
void ParticleSystem::setWind(const glm::vec3& wind) {
    windVelocity = glm::normalize(wind) * 2.0f;
}
...
void ParticleSystem::clear() {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, vboInstance);
    glBufferData(GL_ARRAY_BUFFER, 0, nullptr, GL_DYNAMIC_DRAW); // wyczyść bufor na GPU
    glBindBuffer(GL_ARRAY_BUFFER, 0);
}